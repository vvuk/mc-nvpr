...
{
  gfxWarning() << "ScaledFontNVpr::Create for " << aFont->mName << " style " << (int)aFont->mStyle;
  RefPtr<FontNVpr>& font = fontCache[*aFont];
  if (!font) {
    font = FontNVpr::Create(aFont, aForeignFont);
    gfxWarning() << "... doesn't exist -- created " << font.get() << "\n";
  } else {
    gfxWarning() << "... already exists -- " << font.get() << "\n";
  }
...
FontNVpr::InitFromForeignFont(ScaledFontBase* aForeignFont)
{
  ...
  gfxWarning() << this << " foreign font " << aForeignFont << " type " << aForeignFont->GetType();
  gfxWarning() << this << " created " << mNumGlyphs << " glyphs, first path at " << mGlyphPaths;

  // XXX get a real glyphs bounding box!
  mGlyphsBoundingBox = Rect(-.25, -1.25, 1.25, 1.75);

  return true;
}

void
FontNVpr::InitFromSystemFont(const FontOptions* aFont)
{
  gl->MakeCurrent();
